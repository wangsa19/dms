                ...
                <table class="w-full text-sm">
                    <thead class="bg-gray-50 text-gray-600">
                        <tr>
                            ...
                            {{-- <th class="p-3 text-left font-semibold whitespace-nowrap">Unit ID</th> --}}
                    ...
                    <tbody class="divide-y divide-gray-700 text-gray-700">
                        ...
                        @forelse ($actionFrequencyUnits as $index => $unit)
                        <tr class="hover:bg-gray-50 transition">
                            <td class="p-3 align-middle whitespace-nowrap">{{ $actionFrequencyUnits->firstItem() +
                                $index }}</td>
                            {{-- <td class="p-3 align-middle whitespace-nowrap">{{ $unit->id }}</td> --}}
                            <td class="p-3 align-middle whitespace-nowrap font-medium">{{ $unit->name }}</td>
                        ...
                        <tr>
                            <td colspan="5" class="text-center p-4 text-gray-500">No action frequency units found.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-50 ">
        <div class="bg-white w-full max-w-lg rounded-xl p-6 shadow-lg">
            <h3 class="text-xl font-bold text-gray-800 mb-6">
                {{ $unitId ? 'Edit Action Frequency Unit' : 'Create New Action Frequency Unit' }}
            </h3>
            ...
            <form wire:submit.prevent="save">
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input id="name" wire:model="name" placeholder="Enter unit name"
                    ...
                    <div>
                        ...
                        <input id="code" wire:model="code" type="text" placeholder="Enter unit code (Optional)"
                            class="w-full border border-gray-300 rounded-lg px-3 py-2 mt-1 text-sm focus:ring-2 focus:ring-blue-500">
                        @error('code')
                        <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                        @enderror
    ...
    <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-50 ">
        <div class="bg-white max-w-sm w-full p-6 rounded-xl shadow-lg">
            ...
            <p class="text-sm text-gray-600 mb-6">
                Are you sure you want to delete this action frequency unit? This action cannot be undone.
            </p>
